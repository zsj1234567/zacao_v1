...
class MainWindow(QMainWindow):
    ...
    def __init__(self, log_file_path=None):
        ...
        self.db_auto_refresh_timer = None
        # 初始化动态分析管理器
        ...
        self.db_pass_edit = QLineEdit("123456")
        self.db_pass_edit.setEchoMode(QLineEdit.EchoMode.Password)#密码隐藏
        ...
        self.db_connect_button.clicked.connect(self.on_db_connect_and_load)#连接并且加载数据
        ...
        # 添加按设备号分析按钮
        self.device_analysis_button = QPushButton("按设备号分析")
        self.device_analysis_button.clicked.connect(self.on_device_analysis_clicked)
        self.device_analysis_button.setObjectName("device_analysis_button")
        self.device_analysis_button.setStyleSheet("background-color: #28a745; color: white; font-weight: bold;")
        self.device_analysis_button.setEnabled(False)  # 初始状态禁用，需要先连接数据库
        db_params_layout.addRow(self.device_analysis_button)
        
        self.db_params_group.setLayout(db_params_layout)
        self.db_params_group.setVisible(False)  # 保持隐藏
        # io_layout.addWidget(self.db_params_group)  # 注释掉，不再添加到界面   addwidget添加到控制面板
        # 输入路径
       ...
       #当动态分析完成时，自动触发数据库写入操作
        self.dynamic_analysis_manager.analysis_complete_signal.connect(self.write_analysis_results_to_db)
...
#TODO 手动分析过程 111
    def start_analysis(self):
        ...
        if self.is_database_mode() and hasattr(self, 'db_manager') and hasattr(self, 'db_loaded_image_paths'):
            ...
            # 使用已经在UI中建立的映射关系
            image_to_ld_lid = self.image_to_ld_lid
            
            # 为每个图像创建单独的配置，确保每个图像使用正确的ld_lid
            all_configs = []
            for img_path in self.db_loaded_image_paths:
                img_config = config.copy()
                img_config['input_paths'] = [img_path]  # 只分析一张图片
                
                # 获取对应的ld_lid
                ld_lid = image_to_ld_lid.get(img_path)
                if ld_lid is not None:
                    img_config['ld_lid'] = ld_lid
                    self.append_log(f"为图像 {os.path.basename(img_path)} 配置ld_lid={ld_lid}")
                else:
                    self.append_log(f"警告: 图像 {os.path.basename(img_path)} 没有对应的雷达数据")
                
                all_configs.append(img_config)
            
            # 保存最后一个配置作为last_run_config
            self.last_run_config = config.copy()
            
            # 创建一个合并的配置，用于分析所有图像
            merged_config = config.copy()
            merged_config['all_configs'] = all_configs
            
            # 不将db_manager放入config，直接作为参数传递
            self.analysis_runner = AnalysisRunner(merged_config, db_manager_ref=self.db_manager)
        ...
        if not actual_image_paths_list:
            ...
            return
     #将变量 actual_image_paths_list 的值赋给配置字典 config 中的 input_paths 键
...
#todo  """从analysis_summary.json文件中加载所有结果图像"""
    ...
    def write_analysis_results_to_db(self, results_list):
        ...
        参数:
            results_list: 分析结果列表或单条分析字典。
        """
        if not (hasattr(self, 'db_manager') and self.db_manager):
            self.append_log("[DB写入] 未检测到数据库连接，跳过写入数据库。")  # 检查数据库连接是否存在
            return
        db = self.db_manager
     ...
     # 获取图片路径到lid的映射关系
        ...
        def get_str(key, default='N/A'):
            v = image_result.get(key, default)
        ...
        for image_result in results_list:
            ...
            if jpeg_lid is None or ld_lid is None:
                ...
                continue
            # 保证分析结果有标准字段
            grass_coverage = get_str('草地盖度')
            grass_height = get_str('草地高度')
            grass_density = get_str('草地密度')
            # 将结果转换为数值类型
            coverage = extract_number(grass_coverage)
            height = extract_number(grass_height)
            density = extract_number(grass_density)
            ...
            '''
            #动态分析复选框未勾选时执行数据库写入
            if not self.dynamic_analysis_checkbox.isChecked():  
                try:
        ...
        # 不自动切换到结果图像，仅刷新原始图像视图
        if results_list:
            first_path = results_list[0].get('original_path')
            if first_path:
                self.image_viewer._set_view_mode(0, first_path)
    ...
    def on_analysis_complete(self, success, message):
        ...
        #todo 数据库写入逻辑
        results_list = []
        if self.is_database_mode() and hasattr(self, 'db_manager'):   #检查是否处于数据库模式且已初始化数据库管理器
            try:
                results_list = json.loads(message)
            except Exception:
                results_list = []
            if isinstance(results_list, list) and results_list:        # 验证结果是否为非空列表
                self.write_analysis_results_to_db(results_list)         # 将分析结果写入数据库

        # After analysis, regardless of success/fail, re-enable inputs    无论分析成功或失败，都重新启用输入控件
        self.set_inputs_enabled(True) # Make sure inputs are re-enabled   确保输入框可再次使
    ...
    def set_inputs_enabled(self, enabled):
        ...
        # 数据库相关按钮
        if hasattr(self, 'device_analysis_button'):
            # 只有在数据库模式下且数据库已连接时才启用按设备号分析按钮
            db_connected = hasattr(self, 'db_manager') and self.db_manager is not None
            self.device_analysis_button.setEnabled(enabled and self.is_database_mode() and db_connected)
    ...
    def toggle_dynamic_analysis(self, state):
        ...
        if state == Qt.CheckState.Checked.value:
            # 数据库模式下自动连接数据库
            if self.is_database_mode() and not hasattr(self, 'db_manager'):
                self.append_log("动态分析前自动连接数据库...")
                self.on_db_connect_and_load(auto_connect_db=True)
            # 获取当前配置
            config = self.get_config()
            if not config:
                self.dynamic_analysis_checkbox.setChecked(False)
            ...
            if self.is_database_mode() and hasattr(self, 'db_manager'):
                self.dynamic_analysis_manager.set_db_manager(self.db_manager)
                self.dynamic_analysis_manager.set_monitor_mode('database')
            else:
                self.dynamic_analysis_manager.set_monitor_mode('local')
            # 设置检查间隔
    ...
    def on_dynamic_analysis_all_completed(self):
        ...
        # 恢复按设备号分析按钮状态
        if hasattr(self, 'device_analysis_button'):
            self.device_analysis_button.setEnabled(True)
            self.device_analysis_button.setText("按设备号分析")
    ...
    def switch_to_latest_image(self):
        ...
        if self.latest_image_path and os.path.exists(self.latest_image_path):
            ...
            self.image_viewer.view_original_button.setChecked(True)
            self.image_viewer._set_view_mode(0, target_image_path=self.latest_image_path)
#todo """切换本地/数据库模式时，显示/隐藏相关控件，并重置动态分析状态"""
    def on_source_mode_changed(self):
        """切换本地/数据库模式时，显示/隐藏相关控件，并重置动态分析状态"""
        ...
        if self.dynamic_analysis_checkbox.isChecked():
            ...
            self.start_button.setEnabled(True)
            self.stop_button.setEnabled(False)
            self.append_log("切换输入源后，动态分析已自动关闭，请根据需要重新开启。")
#todo 连接数据库并加载图片到预览列表（自动读取配置文件）
    def on_db_connect_and_load(self, auto_connect_db=False):
        """连接数据库并加载图片到预览列表（自动读取配置文件）"""
        try:
            from utils.db_connection import load_db_config
            ...
            QMessageBox.critical(self, "数据库配置读取失败", str(e))
            return
        try:
        ...
        try:
            ...
            # 创建LD LID集合，用于快速查找 雷达id集合
            ld_lid_set = {rec['lid'] for rec in ld_records}
            for rec in jpeg_records:
                jpeg_lid = rec['lid']
                # 保存JPEG图像到文件
                img_path = self.db_manager.save_jpeg_to_file(jpeg_lid, output_dir=temp_dir)
                image_paths.append(img_path)
                self.lid_to_image[jpeg_lid] = img_path
                self.image_to_lid[img_path] = jpeg_lid
                self.image_to_jpeg_lid[img_path] = jpeg_lid
                # #todo 不需要对应
                # 检查是否有相同lid的LD数据
          #       if jpeg_lid in ld_lid_set:
          #           # 使用相同的lid进行配对
          #           self.image_to_ld_lid[img_path] = jpeg_lid
          #           self.append_log(f"图像 {os.path.basename(img_path)} 配对到雷达数据 lid={jpeg_lid}")
          #       else:
    ...
    def _start_daily_auto_load(self):
        """启动每日定时加载功能"""
        # 如果已经有定时器在运行，先停止它
        if self.db_auto_refresh_timer and self.db_auto_refresh_timer.isActive():
            self.db_auto_refresh_timer.stop()

        # 创建新的定时器
        self.db_auto_refresh_timer = QTimer(self)
        self.db_auto_refresh_timer.timeout.connect(self._on_daily_auto_load)

        # 计算今天4点的时间
        now = datetime.datetime.now()
        target_time = now.replace(hour=21, minute=37, second=0, microsecond=0)
        ...
        # 设置单次触发定时器，到达4点时触发首次加载
        self.db_auto_refresh_timer.setSingleShot(True)
        self.db_auto_refresh_timer.start(time_diff)

        # 记录日志或显示提示
        print(f"已设置每日自动加载，下次加载时间: {target_time.strftime('%Y-%m-%d %H:%M:%S')}")

    def _on_daily_auto_load(self):
        """每日定时加载回调函数"""
        try:
            print("开始执行每日自动加载数据...")
            # 执行数据加载操作
            ...
            # 设置下一次自动加载 (24小时后)
            if self.db_auto_refresh_timer:
                self.db_auto_refresh_timer.start(24 * 60 * 60 * 1000)  # 24小时
                next_load_time = datetime.datetime.now() + datetime.timedelta(days=1)
                print(f"下一次自动加载时间: {next_load_time.strftime('%Y-%m-%d %H:%M:%S')}")
        except Exception as e:
            print(f"每日自动加载失败: {str(e)}")
            # 重新设置定时器，确保即使失败也会尝试下一次加载
            if self.db_auto_refresh_timer:
                self.db_auto_refresh_timer.start(24 * 60 * 60 * 1000)

    def on_device_analysis_clicked(self):
        """
        按设备号分析按钮点击事件处理函数
        调用DynamicAnalysisManager的analyze_all_by_device方法进行批量分析
        """
        if not hasattr(self, 'db_manager') or not self.db_manager:
            QMessageBox.warning(self, "数据库未连接", "请先连接数据库并加载数据")
            return
            
        if not hasattr(self, 'db_loaded_image_paths') or not self.db_loaded_image_paths:
            QMessageBox.warning(self, "数据未加载", "请先加载数据库数据")
            return
        ...
        reply = QMessageBox.question(
            self, 
            "确认分析", 
            "即将开始按设备号批量分析所有图片盖度和雷达高度，这可能需要较长时间。\n\n确定要继续吗？",
        ...
        try:
            # 禁用按钮，防止重复点击
            self.device_analysis_button.setEnabled(False)
            self.device_analysis_button.setText("分析中...")
            ...
            self.append_log("开始按设备号批量分析...")
            self.append_log(f"输出目录: {output_dir}")
            
            # 启动分析
            self.dynamic_analysis_manager.analyze_all_by_device(output_dir=output_dir)
            
        except Exception as e:
            QMessageBox.critical(self, "分析启动失败", f"启动按设备号分析时出错：\n{str(e)}")
            self.append_log(f"分析启动失败: {str(e)}")
            # 恢复按钮状态
            self.device_analysis_button.setEnabled(True)
            self.device_analysis_button.setText("按设备号分析")
