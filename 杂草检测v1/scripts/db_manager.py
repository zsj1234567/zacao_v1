...
class PostgresManager:
    ...
    def get_jpeg_data_by_lqn_lct(self, lqn: str, lct: str) -> List[Dict[str, Any]]:
        """
        按lqn和lct查询图片数据（nk_ope_data_lc301b_jpeg）
        """
        query = "SELECT * FROM nk_ope_data_lc301b_jpeg WHERE lqn = %s AND lct::date = %s"
        return self.execute_query(query, (lqn, lct))

    def get_eq_data_by_lqn_lct(self, lqn: str, lct: str) -> List[Dict[str, Any]]:
        """
        按lqn和lct查询雷达数据（nk_open_data_lc301b_eq）
        """
        query = "SELECT * FROM nk_open_data_lc301b_eq WHERE lqn = %s AND lct::date = %s"
        return self.execute_query(query, (lqn, lct))
    ...
    def get_image_to_ld_lid_map(self, output_dir="output_images"):
        ...
        # 创建一个从 lid 到 ld 记录的映射，以便高效查找
        ld_lid_map = {rec['lid']: rec for rec in ld_records}

        image_to_ld_lid = {}
        for jpeg_rec in jpeg_records:
            jpeg_lid = jpeg_rec['lid']
            img_path = self.save_jpeg_to_file(jpeg_lid, output_dir=output_dir)

            # 查找是否有相同 lid 的 ld 记录
            if jpeg_lid in ld_lid_map:
                image_to_ld_lid[img_path] = jpeg_lid  # 使用相同的 lid 进行配对
            else:
                image_to_ld_lid[img_path] = None
        return image_to_ld_lid

    def get_analyzed_jpeg_lids(self) -> set:
        """
        获取nk_analysis_results表中所有已分析的jpeg_lid集合
        """
        try:
            results = self.execute_query("SELECT DISTINCT jpeg_lid FROM nk_analysis_results")
            return set(row['jpeg_lid'] for row in results if row['jpeg_lid'] is not None)
        except Exception as e:
            logging.error(f"获取已分析jpeg_lid失败: {e}")
            return set()
